...
function main(): void {
    const shapeHandler: ShapeHandler = new ShapeHandler()
    ...
    readLineInterface.on('line', (line: string) => {
        ...
        try {
            shapeHandler.addShape(line)
        } catch (error) {
            printMessage(error.message)
        }
    }).on('close', () => {
        const maxAreaShape: string = shapeHandler.getMaxAreaShape()
        printMessage(`${MAX_AREA_SHAPE}${maxAreaShape}`)

        const minPerimeterShape: string = shapeHandler.getMinPerimeterShape()
        printMessage(`${MIN_PERIMETER_SHAPE}${minPerimeterShape}`)

        shapeHandler.createImage()
...
function isRectangleDescriptionCorrect(lineArray: string[]): boolean {
...
function isTriangleDescriptionCorrect(lineArray: string[]): boolean {
...
function isCircleDescriptionCorrect(lineArray: string[]): boolean {
...
function isLineDescriptionCorrect(lineArray: string[]): boolean {
...
function isNotColor(lineItem: string): boolean {
